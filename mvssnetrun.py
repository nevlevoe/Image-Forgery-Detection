...
model_path = r"./defactomvssnet.pt"
image_path = r""
...
def calculate_pixel_f1(pred, gt):
    """Calculate pixel-level F1 score between prediction and ground truth."""
    eps = 1e-8
...
binary_pred = (mask > 0.5).astype(np.uint8)

resized_img = img.resize((512, 512), Image.BILINEAR)

# Display results
plt.figure(figsize=(20, 5))
plt.subplot(1, 4, 1); plt.title("Resized Input (512×512)"); plt.imshow(resized_img); plt.axis('off')
plt.subplot(1, 4, 2); plt.title("Predicted Mask"); plt.imshow(mask, cmap='gray', vmin=0, vmax=1); plt.axis('off')
plt.subplot(1, 4, 3); plt.title("Binarized Mask"); plt.imshow(binary_pred * 255, cmap='gray', vmin=0, vmax=255); plt.axis('off')
plt.tight_layout()
plt.show()